...
from utils import phone_saver, show_all_for_name, show_all_for_phone, show_all_for_email, show_all_for_phones, delete_by_id
...
class AddressBookApp(MDApp):
    ...
    def show_all(self):
        ...
        for contact in show_all_for_phones():
            result_list_widget.add_widget(
                SearchResultsItem(text=f'{contact[0]}', secondary_text=f'{contact[1]}', user_id=contact[2])
            )
    ...
    def save_contact_and_switch_to_search(self, name, phones, email, addresses, birthday):
